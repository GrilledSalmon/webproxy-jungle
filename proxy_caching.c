...
/* 각 cache의 정보가 담길 구조체 */
typedef struct {
    char *url;      /* url이 담김(나중에 chace를 뒤질 때 url을 확인) */
    int *flag;      /* 캐시가 비어있는지 차 있는지(0 or 1) */
    int *cnt;       /* 최근 방문 순서 나타내기 위한 변수 -> 방문하면(cache hit) 1로 초기화 */
...
int main(int argc, char **argv) {
    ...
    // 반복문 한 번에 client request 하나, thread 하나.
    while (1) {
        clientlen = sizeof(clientaddr);                                                 /* 소켓 구조체 크기 */
        connfd = Malloc(sizeof(int));                                                   // thread들끼리의 경쟁상태(race)로 인해 같은 위치의 connfd를 수정할 수 있으므로 malloc으로 아예 할당
        *connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);                      /* 연결 요청 큐에 아무것도 없을 경우 기본적으로 연결이 생길때까지 호출자를 막아둠, 즉 대기 상태 */
        ...
        Pthread_create(&tid, NULL, thread, connfd);                                     /* thread 생성, 트랜잭션은 thread가 수행해줌. thread가 할당되면 이 line은 끝남(thread가 끝나길 기다리지 않음)*/
    }
}



// thread의 routine(실행할 함수) -> 각각의 쓰레드가 connfd에 대해 doit을 시행해줌
...
// 각각의 thread가 connfd에 대해 client의 request를 듣고 request에 있는 url이 캐시에 있나 확인
// 캐시 hit가 되면 프록시가 바로 response를 보내주고, cache miss가 되면 서버에 request 보내고
// 서버의 response를 캐시에 저장(이 때 LRU(Least Recently Used) 사용)
void doit(int connfd) {
    ...
    if(reader(connfd, url)) {
        return; // cache hit가 일어나면 이후(서버에 request 보내기)는 일어나지 않음(프록시 단에서 처리 끝)
    }

    // 이하 cache miss가 일어났다면~~~~~~~~
    /* ip 정보, 경로와 port번호 세팅해줄 함수 */
    ...
    while((n = rio_readlineb(&server_rio, buf, MAXLINE)) != 0) {
        ...
        if (total_size + n < MAX_OBJECT_SIZE) {
            strcpy(content_buf + total_size, buf); // content_buf에 total_size를 더해서 이미 차있는 부분 다음부터 작성
        }
        total_size += n;
    }

    /* cache content의 최대 크기를 넘지 않았다면 cache에 쓰기. 이거 근데 애초에 위에서 체크하지 않나..? */
...
void init_cache() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        *(cache[i].cnt) = 1;                                                /* cnt 1으로 설정, 최근에 찾은 것일 수록 1이랑 가까움 */
    }
}
...
int reader(int connfd, char *url) {
    ...
    P(&mutex);              // readcnt 수정하겠습니다~ 아무도 건드리지 마세요~
    readcnt++;
    if(readcnt == 1) {  // readcnt가 1일 때(누군가 읽기를 시작했다면) w문이 닫힌다.(cache의 write를 제한한다)
        P(&w);          // readcnt는 현재 cache를 읽고 있는 thread가 몇개이냐를 의미
    }
    V(&mutex); // readcnt 수정 완료했습니다~

    /* cache를 다 돌면서 cache에 써있고 cache의 url과 현재 요청한 url이 같으면 client fd에 cache의 내용 쓰고 해당 cache의 cnt를 0으로 초기화 후 break */
    for(int i = 0; i < 10; i++) {
        if(*(cache[i].flag) == 1 && !strcmp(cache[i].url, url)) { // cache가 들어 있고, cache의 url과 target url이 같다면(cache hit!)
            Rio_writen(connfd, cache[i].content, MAX_OBJECT_SIZE); // 컨텐츠의 내용을 connfd에 작성
            return_flag = 1;
            *(cache[i].cnt) = 1; // cache hit가 난 cache의 cnt를 1로 초기화
            break;
        }
    }
    
    /* 모든 cache객체의 cnt를 하나씩 올려줌, 즉, 방문안한 횟수를 올려줌 */
    for(int i = 0; i < 10; i++) {
        (*(cache[i].cnt))++;
    }

    P(&mutex); // readcnt 수정하겠습니다~
    readcnt--;
    if(readcnt == 0) { // cache를 읽고 있는 thread가 더이상 없다면
        V(&w); // write해도 됩니다.
    }
    V(&mutex); // readcnt 수정 완료했습니다~
    return return_flag; // cache hit가 일어났는지 여부
}
...
void writer(char *url, char *buf) {
    ...
    for(int i = 0; i < 10; i++) { // 10개의 cache에 대해
        if(*(cache[i].flag) == 0) { // 비어있는 cache가 있다면 for문 종료
            idx = i;
            break;
        }
        if(*(cache[i].cnt) > max_cnt) { 
            idx = i;  // cnt가 max인 index 기억
            max_cnt = *(cache[i].cnt);
        }
    }
    /* 해당 index에 cache 작성 */
